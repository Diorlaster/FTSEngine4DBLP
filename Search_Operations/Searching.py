...
def search(indexes, user_output_results):
    ...
    user_splitted_query = get_queries(user_query)
    user_whoosh_query = get_whoosh_query(user_splitted_query);
    ...
    print_results(results_publications, user_output_results)
...
def get_whoosh_query(user_splitted_query):
    user_whoosh_query = ""
...
def print_results(results_publications, user_output_results ):
    ...
    if len(results_publications) > 0:
        print(Fore.BLUE + "Sono stati trovati " + Style.BRIGHT + Fore.BLACK + str(
            len(results_publications)) + Style.RESET_ALL + Fore.BLUE + " risultati!\n")
